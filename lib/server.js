...
const protocol = function(self, message) {
  ...
  // Switch on type
  if (type === opcodes.OP_UPDATE) return new Update(message);
  if (type === opcodes.OP_INSERT) return new Insert(message);
  if (type === opcodes.OP_QUERY) return new Query(message);
  if (type === opcodes.OP_GETMORE) return new GetMore(message);
  if (type === opcodes.OP_DELETE) return new Delete(message);
  if (type === opcodes.OP_KILL_CURSORS) return new KillCursor(message);
